      ...
      if (this.$route.params.id !== '-1') {
        ...
        axios.post('/api/securitymonitor/findByParam', (this.user.userId, form))
          .then(res => {
            ...
            //  let ProcStatus = this.departmentDOs2.filter(sub => {
            //   return sub.deptId === this.user.deptId
            // })
            // 获取子流程状态
            let subProcStatus = []
            for(var i = 0; i<this.departmentDOs2.length;i++){
              if(this.departmentDOs2[i].leaderDOs && this.departmentDOs2[i].leaderDOs.length>0){
                 subProcStatus =this.departmentDOs2[i].leaderDOs.filter(sub => {
                      if(sub.teamUserId===this.user.userId){
                        return true
                      }else{
                        return false
                      }
                 ...
                 if(subProcStatus.length>0){
                   this.subProcStatus = this.departmentDOs2[i].status
                   break
                 }
              }
            }
            // this.ProcStatus = ProcStatus.length > 0 ? ProcStatus[0].status : null
            let querySave = {
